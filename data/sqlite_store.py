...
def save_snapshot(user_id, scene, choices, history):
    ...
    with _connect() as conn:
        conn.execute("""
        INSERT INTO story_progress(user_id, scene, choices, history, decisions_count, updated_at)
        VALUES (?, ?, ?, ?, ?, datetime('now'))
        ...
        """, (user_id, scene, json.dumps(choices), json.dumps(history), decisions_count))
        conn.commit()

def load_snapshot(user_id):
    with _connect() as conn:
        cur = conn.execute("SELECT scene, choices, history, decisions_count FROM story_progress WHERE user_id=?", (user_id,))
        row = cur.fetchone()
        if not row:
            return None
